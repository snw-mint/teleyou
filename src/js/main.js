...
/**
 * Blends primary (8%) into surface (92%) and returns a fully opaque hex.
 * Using JS math instead of CSS color-mix() avoids alpha channel issues
 * that some browsers produce with very low mix percentages.
 */
function calcCardBg(primaryHex, surfaceHex) {
  const parse = (h) => [
    parseInt(h.slice(1, 3), 16),
    parseInt(h.slice(3, 5), 16),
    parseInt(h.slice(5, 7), 16),
  ];
  const [pr, pg, pb] = parse(primaryHex);
  const [sr, sg, sb] = parse(surfaceHex);
  const r = Math.round(pr * 0.08 + sr * 0.92);
  const g = Math.round(pg * 0.08 + sg * 0.92);
  const b = Math.round(pb * 0.08 + sb * 0.92);
  return `#${r.toString(16).padStart(2, '0')}${g.toString(16).padStart(2, '0')}${b.toString(16).padStart(2, '0')}`;
}

/**
 * Returns a readable foreground color (black or white) for a given
 * hex background color. Used to keep the header logo always visible
 * even when the user edits individual palette roles.
 */
...
/**
 * Updates the header logo icon color to always contrast against
 * the current --m3-primary background of the logo shape.
 */
function updateLogoIconContrast() {
  const logoIcon = document.querySelector('.brand-logo-svg');
  if (!logoIcon) return;
  const primaryHex = getComputedStyle(document.documentElement)
    .getPropertyValue('--m3-primary').trim();
  if (!primaryHex) return;
  logoIcon.style.color = getContrastColor(primaryHex);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const applyTheme = (isDark) => {
    ...
    if (currentExtractedTheme) applyMaterialThemeToUI(currentExtractedTheme);
  };
  ...
  themeBtn.addEventListener('click', () => {
    applyTheme(rootElement.getAttribute('data-theme') !== 'dark');
  });
  ...
  wallItems.forEach(item => {
    item.addEventListener('click', () => {
      ...
      requestAnimationFrame(() => requestAnimationFrame(() => extractThemeFromImage(url)));
    });
  });
  ...
  imageUploadInput.addEventListener('change', (event) => {
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => extractThemeFromImage(imageUrl)));
  });
  ...
  randomizeColorBtn.addEventListener('click', () => {
    ...
    currentExtractedTheme = themeFromSourceColor(argbFromHex(randomHex));
  ...
  if (infoBtn) {
    ...
    const onInfoKeyDown = (e) => { if (e.key === 'Escape') closeInfoModal(); };
  ...
  const showTooltip = (e) => {
    ...
    let top  = placeBelow ? rect.bottom + 8 : rect.top - tooltipRect.height - 8;
    let left = rect.left + (rect.width / 2) - (tooltipRect.width / 2);
    if (!placeBelow && top < 8) top = rect.bottom + 8;
    left = Math.max(8, left);
    const rightOverflow = left + tooltipRect.width - window.innerWidth + 8;
    if (rightOverflow > 0) left -= rightOverflow;
    tooltipElement.style.top  = `${top}px`;
    tooltipElement.style.left = `${left}px`;
  };
  const hideTooltip = () => tooltipElement.classList.remove('visible');
  ...
  // ── HCT Picker — individual palette role editing ──
  const PALETTE_ROLES = [
  ...
  const hctPicker = createHctPicker({
    onchange(argb) {
      ...
      if (activePaletteRole.cssVar) {
        ...
        // Keep logo icon readable against the (possibly changed) primary
        updateLogoIconContrast();
        // Recompute card background if primary changed
        if (activePaletteRole.cssVar === '--m3-primary') {
          const surfaceHex = getComputedStyle(document.documentElement)
            .getPropertyValue('--m3-surface').trim();
          document.documentElement.style.setProperty(
            '--m3-card-bg',
            calcCardBg(hexFromArgb(argb), surfaceHex)
          );
        }
      } else {
  ...
  // Wire up individual palette role dots
  document.querySelectorAll('.palette-list .palette-card').forEach((card, i) => {
    ...
    dot.addEventListener('keydown', e => {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openPickerForRole(); }
    });
  });

  // Wire up source color dot
  ...
  if (sourceColorDot) {
    ...
    sourceColorDot.addEventListener('keydown', e => {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openPickerForSource(); }
    });
  }
...
// ─── Apply theme to mockup CSS variables ─────────────────────────────────────
...
// ─── Apply full Material theme to UI ─────────────────────────────────────────
...
function applyMaterialThemeToUI(theme) {
  ...
  const nvHex = hexFromArgb(theme.palettes.neutralVariant.tone(isDark ? 80 : 40));
  const nvR = parseInt(nvHex.slice(1, 3), 16);
  const nvG = parseInt(nvHex.slice(3, 5), 16);
  const nvB = parseInt(nvHex.slice(5, 7), 16);
  root.style.setProperty('--m3-segmented-bg', `rgba(${nvR},${nvG},${nvB},0.12)`);
  ...
  // Opaque card background: 8% primary blended into surface, computed in JS
  // to avoid alpha channel issues that CSS color-mix() can produce
  const cardBg = calcCardBg(
    hexFromArgb(scheme.primary),
    hexFromArgb(scheme.surface)
  );
  root.style.setProperty('--m3-card-bg', cardBg);

  applyToMockup(scheme);

  // Update source color indicator
  ...
  // Update header logo icon contrast (full theme change always has correct on-primary)
  updateLogoIconContrast();
...
function getThemeForExport() {
  if (currentExtractedTheme) return currentExtractedTheme;
  const primaryHex = getComputedStyle(document.documentElement)
    .getPropertyValue('--m3-primary').trim() || '#65558F';
...
function updateDynamicFavicon(bgHex, iconHex) {
  ...
  const svgString = `<svg viewBox="0 0 192 192" xmlns="http://www.w3.org/2000/svg" fill="none">
    ...
    <g transform="translate(${iconOffset} ${iconOffset}) scale(${iconScale})">
      <path stroke="${iconHex}" stroke-width="12" stroke-linecap="round" stroke-linejoin="round"
